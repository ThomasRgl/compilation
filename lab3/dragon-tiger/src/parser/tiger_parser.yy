...
%type <Expr *> expr stringExpr intExpr seqExpr callExpr opExpr negExpr
            assignExpr whileExpr forExpr breakExpr letExpr var ifElseExpr ;
...
opExpr: expr PLUS expr   { $$ = new BinaryOperator(@2, $1, $3, o_plus); }
      ...
      | expr AND expr    {
        $$ = new IfThenElse(@2, //loc
                            $1, //Expr condition
                            new IfThenElse(@3, $3, new IntegerLiteral(nl, 1), new IntegerLiteral(nl, 0)), //then
                            new IntegerLiteral(nl, 0)); //Else
      }
      | expr OR expr    {
        $$ = new IfThenElse(@2, $1,
                            new IntegerLiteral(nl, 1), //then
                            new IfThenElse(@3, $3, new IntegerLiteral(nl, 1), new IntegerLiteral(nl, 0)) );  //else
      }
;
